...
const Auth: React.FC<AuthProps> = ({isOpen, onClose}) => {
    ...
    const notLogin = (): string => action === 'Login' ? 'Register' : 'Login'
                                ...
                                <Stack spacing={4}>
                                    ...
                                    <FormControl id={"password"}>
                                        ...
                                        <Input type={"password"} onChange={(e) => setPassword(e.target.value)}
                                               onKeyDown={
                                                   async (k) => {
                                                       if (k.key == 'Enter') await sendAuth()
                                                   }}/>
                                    </FormControl>
                                    <Button bg={'blue.400'} color={'white'} _hover={{bg: 'blue.500'}}
                                            onClick={sendAuth}>{getButtonText()}</Button>
                                </Stack>
                                <Stack pt={6}>
                                    <Text align={'center'} fontSize={'sm'}>{getBottomText()} <Link color={'blue.400'}
                                                                                                   fontSize={'sm'}
                                                                                                   onClick={() => setAction(action === 'Login' ? 'Register' : 'Login')}>{notLogin()}</Link>
                                    </Text>
                                </Stack>
                            </Box>
                        </ModalBody></div> : (
