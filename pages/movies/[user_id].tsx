...
//@ts-ignore
const moviesFetcher = () =>
  fetch(`/api/seen-movies`, { credentials: "include" }).then((res) =>
    res.json()
  );
...
function UserMovies() {
  ...
  if (data.seenMovies && data.seenMovies.length !== 0 && movies.length === 0) {
  ...
  const removeMovie = async (): Promise<void> => {
    ...
    if (resp.status === 200) {
      //TODO: filter seems not working properly, fix and remove reload
      location.reload();
    } else {
      console.error(resp);
    }
  ...
  const refreshMovie = async (): Promise<void> => {
    ...
    >
      ...
      {clickedMovie !== undefined ? (
        <TitleCard
          ...
          setClickedRating={setClickedRating}
          upsertTitle={upsertMovie}
          refreshTitle={refreshMovie}
          removeTitle={removeMovie}
        />
      ) : (
        <></>
      )}
